#!/usr/bin/env python3
"""
Top DFW House Buyers — Keyword Landing Page Generator
Creates /sell-my-house-fast-{city}-tx/ pages targeting exact match search queries.
These complement the clean /city/ pages and target high-intent search terms.
...
# Top 10 highest-value DFW cities by search volume
CITIES = [
    {"name": "Celina",          "slug": "celina",           "kw_slug": "sell-my-house-fast-celina-tx",          "county": "Collin",   "zip": "75009"},
    {"name": "Plano",           "slug": "plano",            "kw_slug": "sell-my-house-fast-plano-tx",           "county": "Dallas",   "zip": "75024"},
    {"name": "Frisco",          "slug": "frisco",           "kw_slug": "sell-my-house-fast-frisco-tx",          "county": "Collin",   "zip": "75034"},
    {"name": "McKinney",        "slug": "mckinney",         "kw_slug": "sell-my-house-fast-mckinney-tx",        "county": "Collin",   "zip": "75070"},
    {"name": "Allen",           "slug": "allen",            "kw_slug": "sell-my-house-fast-allen-tx",           "county": "Collin",   "zip": "75002"},
    {"name": "The Colony",      "slug": "the-colony",       "kw_slug": "sell-my-house-fast-the-colony-tx",      "county": "Denton",   "zip": "75056"},
    {"name": "Wylie",           "slug": "wylie",            "kw_slug": "sell-my-house-fast-wylie-tx",           "county": "Collin",   "zip": "75098"},
    {"name": "Little Elm",      "slug": "little-elm",       "kw_slug": "sell-my-house-fast-little-elm-tx",      "county": "Denton",   "zip": "75068"},
    {"name": "Prosper",         "slug": "prosper",          "kw_slug": "sell-my-house-fast-prosper-tx",         "county": "Collin",   "zip": "75078"},
    {"name": "Rockwall",        "slug": "rockwall",         "kw_slug": "sell-my-house-fast-rockwall-tx",        "county": "Rockwall", "zip": "75087"},
]
...
Write a landing page targeting the exact keyword: "sell my house fast {city['name']} TX"

This page should be slightly different from the main {city['name']} page — focus on:
- Urgency and speed of the process
- Specific reasons people in {city['name']} need to sell fast (relocation, divorce, foreclosure, inherited)
- Local {city['name']} market context
...
- Meta title must include "Sell My House Fast {city['name']} TX"
- Meta title under 60 chars
- Meta description under 160 chars

Return ONLY valid JSON (no markdown):
{{
  "meta_title": "Sell My House Fast {city['name']} TX | Cash Offer 24hrs",
  "meta_description": "...",
  "h1": "Sell My House Fast in {city['name']}, TX",
  "intro": "2-3 sentences about selling fast in {city['name']}",
  "content_html": "HTML with h2, p, ul, li tags",
  "why_sellers_title": "Why {city['name']} Homeowners Sell Fast",
  "why_sellers_points": ["...", "...", "...", "..."]
}}"""
...
{{
  ...
  "mainEntity": [
    ...
    {{"@type":"Question","name":"Do I need repairs before selling?","acceptedAnswer":{{"@type":"Answer","text":"Never. We buy houses in {city['name']} in any condition."}}}},
...
:root{{--green:#4ab840;--dark:#1a1f1a;--cream:#f8faf8;--border:#d4e4d2;--muted:#52675f}}
body{{background:var(--cream);color:var(--dark);font-family:'Montserrat',sans-serif;font-weight:300;line-height:1.6}}
.site-nav{{background:#0a0a0a;padding:14px 32px;display:flex;align-items:center;justify-content:space-between;border-bottom:3px solid var(--green);position:sticky;top:0;z-index:100}}
...
.hero{{background:var(--dark);padding:56px 40px;position:relative;overflow:hidden}}
...
.hero h1{{font-family:'Playfair Display',serif;font-size:clamp(28px,4vw,48px);font-weight:900;color:#fff;line-height:1.1;margin-bottom:16px}}
...
.form-headline{{font-family:'Playfair Display',serif;font-size:20px;font-weight:700;color:var(--dark);margin-bottom:4px}}
...
.content-wrap{{max-width:900px;margin:0 auto;padding:56px 24px}}
.content-wrap h2{{font-family:'Playfair Display',serif;font-size:26px;font-weight:700;color:var(--dark);margin:32px 0 12px;line-height:1.2}}
.content-wrap p{{font-size:15px;line-height:1.9;color:#3a4a3a;margin-bottom:14px}}
.content-wrap ul{{padding-left:20px;margin-bottom:14px}}
.content-wrap li{{font-size:15px;line-height:1.8;color:#3a4a3a;margin:6px 0}}
.why-box{{background:var(--dark);padding:28px 32px;margin:32px 0}}
.why-box h2{{font-family:'Playfair Display',serif;font-size:20px;color:#fff;margin-bottom:16px}}
...
.why-box li::before{{content:'✓';color:var(--green);font-weight:700;flex-shrink:0}}
...
.cta-box h3{{font-size:16px;font-weight:700;color:var(--dark);margin-bottom:8px}}
.cta-box p{{font-size:13px;color:var(--muted);margin-bottom:14px;line-height:1.6}}
.cta-box a{{display:inline-block;background:var(--dark);color:#fff;padding:12px 24px;font-weight:700;font-size:12px;text-decoration:none;border-radius:2px;letter-spacing:0.06em;text-transform:uppercase}}
.alt-link{{text-align:center;margin:32px 0;font-size:13px;color:var(--muted)}}
.alt-link a{{color:var(--green);font-weight:600;text-decoration:none}}
footer{{background:#0a0a0a;color:rgba(255,255,255,0.4);text-align:center;padding:24px;font-size:11px;border-top:3px solid var(--green)}}
footer a{{color:var(--green);text-decoration:none}}
.sticky{{position:fixed;bottom:0;left:0;right:0;background:var(--dark);border-top:2px solid var(--green);padding:12px 20px;display:none;align-items:center;justify-content:space-between;z-index:200}}
...
<section class="hero">
  <div class="hero-inner">
    <div>
      <div class="eyebrow">{city['name']}, {city['county']} County TX · Fast Cash Sale</div>
      <h1>{content['h1'].replace('Fast', '<em>Fast</em>')}</h1>
      <p class="hero-sub">Get a fair cash offer within 24 hours. No repairs, no agent fees, no commissions. Close in as few as 7 days.</p>
      <div class="badges">
    ...
    <div class="hero-form">
      ...
      <div class="form-sub">Selling in {city['name']}? Takes 60 seconds.</div>
      ...
      <div id="form-success" style="display:none;text-align:center;padding:24px">
        ...
        <h3 style="font-family:'Playfair Display',serif;font-size:20px;color:var(--dark);margin-bottom:8px">Got It!</h3>
...
<div class="content-wrap">
  ...
  <div class="cta-box">
    ...
    <p>Get a fair cash offer within 24 hours. No fees, no repairs, no commissions. Close on your timeline.</p>
    <a href="tel:9722849713">Call 972-284-9713 Now</a>
  </div>
  <div class="alt-link">
    Want more details about our {city['name']} home buying process? <a href="/{city['slug']}/">Visit our full {city['name']} page →</a>
  </div>
</div>
<footer>
...
def main():
    print(f"Generating {len(CITIES)} DFW keyword landing pages — {datetime.now().isoformat()}")
    print()
    for i, city in enumerate(CITIES):
        ...
        try:
            ...
            print(f"    ✗ Error: {e}")
    print()
    print("Done! Commit all new folders to GitHub.")
    print()
    print("Also update _redirects to REMOVE these URLs from the redirect list:")
    for city in CITIES:
        print(f"  /{city['kw_slug']}/")
